...
분실물 회수
GitHub Pages → Google Apps Script → 공개 Sheet에 회수 상태 기록
                                  └→ 별도 비공개 Sheet에 전화번호 기록
```
...
note: "선택 입력 사항"
status: "회수 완료"
recoveredAt: "2026-09-19T17:30:00.000Z"
```

기존 행에 `location`이 없으면 웹사이트에는 `미지정`으로 표시됩니다.
`status`가 없는 기존 행은 `보관 중`으로 표시됩니다. 회수자의 전화번호는 이 공개 C열에 저장하지 않습니다.
...
또한 최초 실행 시 소유자만 접근할 수 있는 **`다시, 여기 - 비공개 회수 기록`** 스프레드시트를 별도로 생성합니다. 회수자의 전화번호는 이 비공개 문서에만 저장됩니다.
...
보관 중인 물건의 상세 화면에서 **내 물건 가져가기**를 누르면 다음 과정이 실행됩니다.

```text
전화번호 입력 및 동의 → 비공개 회수 기록 Sheet에 저장
                       → 기존 공개 Sheet C열에 회수 완료 상태 기록
                       → 웹사이트의 회수 완료 카테고리로 이동
```

- 공개 웹사이트에는 전화번호가 표시되지 않습니다.
- 공개 Sheet에는 `status`와 `recoveredAt`만 추가됩니다.
- 전화번호는 `setupProject` 실행 시 생성된 비공개 문서에서만 확인합니다.
- 잘못 회수한 경우 서비스 운영자가 해당 비공개 기록의 전화번호로 연락해야 합니다.
- 기능을 추가한 뒤에는 Apps Script를 **새 버전으로 다시 배포**하고 새 `/exec` 주소를 `config.js`에 반영합니다.
...
- 회수 API도 공개되어 있으므로 실제 운영 시에는 본인 확인 절차나 관리자 승인을 추가하는 것이 안전합니다.
- 회수 전화번호는 목적을 달성한 뒤 삭제하는 등 보유 기간을 정해 관리해야 합니다.
